...
# tasteskill: Anti-Slop Frontend Skill (Modular Architecture)

> **Token Economy Notice**: This skill uses a tiered reference architecture. The core execution loop is preserved below, while deep domain references, install matrices, and code skeletons are dynamically linked in `./references/`.

## 📚 Deep Reference Matrix

| Reference Guide | Content Scope | When to Read |
|---|---|---|
| [`references/dial-matrix.md`](references/dial-matrix.md) | The Three Dials (Variance, Motion, Density), Use-Case Presets & Calibration Levels 1-10 | Tuning visual uniqueness, animation tempo, or information density |
| [`references/design-systems.md`](references/design-systems.md) | Official Design System Maps (M3, Fluent, Carbon, Polaris, Primer, Radix, shadcn), Install Commands & Sources | Choosing or installing official UI packages |
| [`references/engineering-directives.md`](references/engineering-directives.md) | Typography, Color, Materiality, Form, Sticky-Stack/Horizontal-Pan skeletons & Dark Mode protocol | Implementing layout mechanics, scroll animations, or theme locks |
| [`references/anti-slop-guide.md`](references/anti-slop-guide.md) | AI Tells (banned gradients, badges, fake stats, em-dash ban), Reference Vocabulary & Hallmark Critique | Code review, anti-slop audits, or visual quality checks |
...
## 1. THE THREE DIALS & DESIGN SYSTEM QUICK-MAP

Before writing layout code, configure the 3 core dials based on the design read:
1. **`DESIGN_VARIANCE` (1-10)**: 1-3 = Enterprise Strict, 4-6 = Polished Commercial, 7-8 = Editorial/Boutique, 9-10 = Brutalist/Avant-Garde.
2. **`MOTION_INTENSITY` (1-10)**: 1-2 = Static/Corporate, 3-5 = Micro-interactions only, 6-7 = Choreographed scroll, 8-10 = Immersive WebGL.
3. **`VISUAL_DENSITY` (1-10)**: 1-3 = Expansive luxury, 4-6 = Standard marketing, 7-8 = Data dashboard, 9-10 = IDE / Bloomberg terminal.

> For the full dial presets and calibration rubric, consult [`references/dial-matrix.md`](references/dial-matrix.md).
> If the brief calls for an enterprise ecosystem (e.g. GitHub/Shopify/IBM style), adopt official packages per [`references/design-systems.md`](references/design-systems.md).
...
## 4. ENGINEERING & QUALITY DIRECTIVES SUMMARY

1. **Anti-Slop Hard Rules**:
   - 🚫 NEVER generate standard AI purple-cyan glows, identical 3-card feature bento grids with emoji icons, or fake stats ("99.9% uptime", "10k+ users").
   - 🚫 Strictly adhere to the **EM-DASH BAN** in user-facing marketing copy.
   - For complete banned patterns, see [`references/anti-slop-guide.md`](references/anti-slop-guide.md).
2. **Layout & Motion Mechanics**:
   - Rely on CSS Grid with named areas or intrinsic sizing (`minmax()`, `clamp()`).
   - Skeletons for Sticky-Stack, Horizontal-Pan, and Scroll-Reveal are available in [`references/engineering-directives.md`](references/engineering-directives.md).
3. **Theme & Dark Mode**:
   - Lock page theme explicitly. If dark mode is requested, use semantic tokens (`bg-surface-primary`, `text-content-subtle`), never hardcoded hex. Details in [`references/engineering-directives.md`](references/engineering-directives.md).
